...
FuzzyAI uses **classifiers** to evaluate the responses generated by large language models (LLMs). These classifiers are implemented in the `fuzzy/handlers/classifiers` directory and can be customized to suit various evaluation needs.

### Key Features of the Evaluation Process:
- **Modular Design**: Multiple classifiers can be used simultaneously by including the `-c` flag during execution.
- **Purpose-Specific Classifiers**: Each classifier is tailored for specific tasks, such as detecting harmful content, ensuring response accuracy, or identifying unsafe outputs.
- **Extensible Support**: New classifiers can be added by following the guidelines in the [Classifiers Wiki](https://github.com/cyberark/FuzzyAI/wiki/Classifiers).

### Example Classifiers:
- **Cosine-Similarity Classifier**: Compares two outputs by measuring the angle between them.
- **Harmfulness Classifier**: Detects whether a given response contains harmful content.

Refer to the [Wiki documentation](https://github.com/cyberark/FuzzyAI/wiki) for detailed usage examples and setup instructions.
