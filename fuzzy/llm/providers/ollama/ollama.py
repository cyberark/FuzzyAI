...
from utils import run_ollama_pull_command
...
class OllamaProvider(BaseLLMProvider):
    ...
    async def validate_models(self) -> None:
        async with self._session.get(f"{self._base_url}/tags") as response:
            ...
            if target_model not in model_names:
                pulled_model = run_ollama_pull_command(target_model)
                if not pulled_model:
                    raise OllamaProviderException(
                        f"Model {self._model_name} not found in local ollama models, available models: {model_names}")

    @api_endpoint("/generate")  # type: ignore
    async def generate(self, prompt: str, url: str, system_prompt: Optional[str] = None, raw: bool = False,
                       **extra: Any) -> Optional[BaseLLMProviderResponse]:
    ...
    async def chat(self, messages: list[BaseLLMMessage], url: str, system_prompt: Optional[str] = None, **extra: Any) -> \
